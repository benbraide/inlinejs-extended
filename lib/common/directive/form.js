...
exports.FormDirectiveHandler = (0, inlinejs_1.CreateDirectiveHandlerCallback)(names_1.FormDirectiveName, ({ componentId, component, contextElement, expression, argKey, argOptions }) => {
    ...
    let localKey = `\$${names_1.FormDirectiveName}`;
    if (argKey === 'field') {
        let evaluate = (0, inlinejs_1.EvaluateLater)({ componentId, contextElement, expression }), savedData = null;
        return (0, inlinejs_1.UseEffect)({ componentId, contextElement,
            callback: () => evaluate((data) => {
                ...
                if (proxy && !(0, inlinejs_1.GetGlobal)().IsNothing(proxy)) {
                    (0, inlinejs_1.IsObject)(savedData) && Object.keys(savedData).forEach(key => proxy.removeField(key));
                    (0, inlinejs_1.IsObject)(data) && Object.entries(data).forEach(([key, value]) => proxy.addField(key, value));
                    savedData = (0, inlinejs_1.DeepCopy)(data);
                }
            }),
        });
    }
    ...
    let handleEvent = (e) => {
        ...
        let event = new CustomEvent(`${names_1.FormDirectiveName}.submitting`);
        ...
        (0, inlinejs_1.GetGlobal)().GetFetchConcept().Get(url, info).then(res => res.text()).then((res) => {
            let response;
            try {
