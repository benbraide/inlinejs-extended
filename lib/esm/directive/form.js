...
export const FormDirectiveHandler = CreateDirectiveHandlerCallback(FormDirectiveName, ({ componentId, component, contextElement, expression, argKey, argOptions }) => {
    ...
    let fieldExpressions = new Array(), evaluateFieldExpressions = () => {
        fieldExpressions.filter(info => (info.source === contextElement || contextElement.contains(info.source))).forEach((info) => {
            EvaluateLater({ componentId, contextElement: info.source, expression: info.value })(value => (IsObject(value) && (fields = Object.assign(Object.assign({}, fields), value))));
        });
    };
    let handleEvent = (e) => {
        var _a, _b, _c;
        ...
        if (options.upload || options.download || options.duplex) {
            ...
            let doFinal = (res) => ((typeof res === 'string') && handleData(res));
            if (options.upload) {
                doWhile(0);
    ...
    (_b = resolvedComponent.FindElementScope(contextElement)) === null || _b === void 0 ? void 0 : _b.SetLocal(localKey, CreateInplaceProxy(BuildGetterProxyOptions({
        getter: (prop) => {
            var _a;
    ...
    (_c = resolvedComponent.FindElementScope(contextElement)) === null || _c === void 0 ? void 0 : _c.SetLocal(secretKey, CreateInplaceProxy(BuildGetterProxyOptions({
        getter: (prop) => {
