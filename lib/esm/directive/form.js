...
import { FindComponentById, AddDirectiveHandler, CreateDirectiveHandlerCallback, EvaluateLater, GetGlobal, JournalError, JournalTry, AddChanges, BuildGetterProxyOptions, CreateInplaceProxy, UseEffect, IsObject, ToString, BindEvent, ResolveOptions } from "@benbraide/inlinejs";
...
export const FormDirectiveHandler = CreateDirectiveHandlerCallback(FormDirectiveName, ({ componentId, component, contextElement, expression, argKey, argOptions }) => {
    ...
    if (argKey === 'field') {
        ...
        (_a = resolvedComponent.FindElementScope(contextElement)) === null || _a === void 0 ? void 0 : _a.AddUninitCallback(() => {
            var _a;
            let proxy = (_a = FindComponentById(componentId)) === null || _a === void 0 ? void 0 : _a.FindElementLocalValue(contextElement, secretKey, true);
        ...
        if (filtered.length > 0) {
            filtered.forEach((info, index) => {
                EvaluateLater({ componentId, contextElement: info.source, expression: info.value })((value) => {
                    IsObject(value) && (fields = Object.assign(Object.assign({}, fields), value));
    ...
    let handleEvent = (e) => {
        ...
        evaluateFieldExpressions(() => {
            ...
            let handleData = (data) => {
                ...
                contextElement.dispatchEvent(event);
                let router = GetGlobal().GetConcept('router'), after = null;
                if (options.refresh) {
                    ...
                    form.reset();
                    (_a = FindComponentById(componentId)) === null || _a === void 0 ? void 0 : _a.GetBackend().changes.AddNextTickHandler(() => form === null || form === void 0 ? void 0 : form.dispatchEvent(new CustomEvent(`${FormDirectiveName}.reset`)));
                }
                    ...
                    if (response.hasOwnProperty('failed')) {
                        ...
                        Object.entries(IsObject(failed) ? failed : {}).forEach(([key, value]) => {
                            ...
                            if (form && !options.novalidate) { //Set validation error
                                ...
                                let local = (item && ((_a = FindComponentById(componentId)) === null || _a === void 0 ? void 0 : _a.FindElementLocalValue(item, `\$${StateDirectiveName}`, true)));
            ...
            if (options.upload || options.download || options.duplex) {
                ...
                if (options.upload) {
                    doWhile(0);
                    (_a = GetGlobal().GetConcept(ServerConceptName)) === null || _a === void 0 ? void 0 : _a.Upload(url, info.body, info.method).While(doWhile).Final(doFinal);
                }
                else if (options.download) {
                    doWhile(0);
                    (_b = GetGlobal().GetConcept(ServerConceptName)) === null || _b === void 0 ? void 0 : _b.Download(url, info.body, info.method).While(doWhile).Final(doFinal);
                }
                else {
                    doWhile({ isUpload: true, value: 0 });
                    (_c = GetGlobal().GetConcept(ServerConceptName)) === null || _c === void 0 ? void 0 : _c.Duplex(url, info.body, info.method).While(doWhile).Final(doFinal);
                }
            }
            else {
    ...
    (_c = resolvedComponent.FindElementScope(contextElement)) === null || _c === void 0 ? void 0 : _c.SetLocal(localKey, CreateInplaceProxy(BuildGetterProxyOptions({
        getter: (prop) => {
            var _a;
            if (prop && state.hasOwnProperty(prop)) {
                (_a = GetGlobal().GetCurrentProxyAccessStorage()) === null || _a === void 0 ? void 0 : _a.Put({ componentId, path: `${id}.${prop}` });
                return state[prop];
            }
            if (prop === 'element') {
    ...
    (_d = resolvedComponent.FindElementScope(contextElement)) === null || _d === void 0 ? void 0 : _d.SetLocal(secretKey, CreateInplaceProxy(BuildGetterProxyOptions({
        getter: (prop) => {
